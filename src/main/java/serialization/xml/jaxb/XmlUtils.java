...
public class XmlUtils {
  ...
  public static Document parseXmlFile(String filename, boolean validating) {
    try {
      ...
      System.err.println("ParserConfigurationException in parsing XML File:" + filename + ", will return null doc");
      //e.printStackTrace();
    } catch (IOException e) {
  ...
  public static Element[] applyXPath(Node node, String xpath) {
    ...
    try {
      ...
      System.err.println("TransformerException in applying xpath:" + xpath + ", will return null array of elements");
      e.printStackTrace();
    }
    return elements;
  }
  ...
  public static Element getUniqueChildNodeFromXPath(Document doc, String xpath) {
    ...
    try {
      ...
      System.err.println("TransformerException in applying xpath:" + xpath + ", will return null element");
      e.printStackTrace();
    }
    return elements;
  }
  ...
  private void addSignature(Element wsSecurityHeaderElement, Element tsElement, Element bodyElement)
      throws NoSuchAlgorithmException, InvalidAlgorithmParameterException, javax.xml.crypto.dsig.XMLSignatureException,
      NoSuchProviderException, javax.xml.crypto.MarshalException {
    ...
    Reference tsReference = xmlSignatureFactory.newReference("#TS",
        ...
        Collections.singletonList(xmlSignatureFactory.newTransform(CanonicalizationMethod.EXCLUSIVE, tsTransformSpec)), null,
        null);
    ...
    if (this.signBody) {
      ...
      Reference bodyReference = xmlSignatureFactory.newReference("#Body",
          xmlSignatureFactory.newDigestMethod(this.digestAlgorithm, null),
          Collections.singletonList(xmlSignatureFactory.newTransform(CanonicalizationMethod.EXCLUSIVE, bodyTransformSpec)), null,
          null);
      references.add(bodyReference);
    }
    
    if (this.signBinarySecurityToken) {
      Reference bstReference = xmlSignatureFactory.newReference("#X509", xmlSignatureFactory.newDigestMethod(
          this.digestAlgorithm, null), Collections.singletonList(xmlSignatureFactory.newTransform(
          CanonicalizationMethod.EXCLUSIVE, (TransformParameterSpec) null)), null, null);
    ...
    Element securityTokenReferenceElement = document.createElementNS(WSSE_NAMESPACE, "wsse:SecurityTokenReference");
    ...
    KeyInfo keyInfo = keyInfoFactory.newKeyInfo(Collections.singletonList(new DOMStructure(securityTokenReferenceElement)));
